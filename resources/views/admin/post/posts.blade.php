...
@section('title','عرض '.$postInfo['title'])
       ...
       <div class="portlet light bordered">
              ...
              <div class="portlet-title">
                     <div class="caption font-red-sunglo">
                            ...
                            <span class="caption-subject bold uppercase"> {{ trans('mainpage.view_posts') }}</span>
                     </div>
              </div>
              <div class="portlet-body form">
                            ...
                            <table class="table table-bordered datatable" id="myTablesw">
                                   <thead>
                                          <tr>
                                                 ...
                                                 <th>{{ trans('mainpage.address_posts') }}</th>
                                                 <th>{{ trans('mainpage.menu') }}</th>
                                                 <th>{{ trans('mainpage.image_post') }}</th>
                                                 <th>{{ trans('mainpage.user') }}</th>
...
<script type="text/javascript">
       $(document).ready(function() {
              var oTable =  $('.datatable').DataTable({
                     ...
                     serverSide: true,
                     ajax: {
                            url: "{{ url('/adminpanel/'.$postInfo['service'].'/getdata') }}" ,
                            data: function (d) {
                     ...
                     columns: [
                            ...
                            {data: 'image', render: function(data){
                                          var items = data;
                                          var item = '{{  url("media/post/gallery/") }}';
                     ...
                     "fnDrawCallback" : function(settings, json){
                            $('.make-switch').bootstrapSwitch({
                                   ...
                                   onSwitchChange: function (event, state) {
                                          ...
                                          $.ajax({
                                            url: "{{ url('/adminpanel/'.$postInfo['service'].'/changestatus') }}/" + id + "/"+state,
                                            type: "get",
              ...
              jQuery(document).ready( function ($){
                     $("#myTablesw tbody").sortable({
                            ...
                            stop: function(en , ui){
                                   ...
                                   $.ajax({
                                          url: "{{ url('/adminpanel/'.$postInfo['service'].'/resort_table') }}",
                                          type: "post",
                                          data: {
                                                 "_token":"{{ csrf_token() }}",
                                                 "item":item
