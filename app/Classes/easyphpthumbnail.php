...
class easyphpthumbnail {
	...
	function easyphpthumbnail() {
		...
		$this->Addtext[0]                = array(0,'Text','50% 50%','',2,'#000000');
		$this->Rotate                 = 0;
	...
	function Createthumb($filename="unknown",$output="screen") {
		...
		if (is_array($filename) && $output=="file") {
			...
			if ($output=="file") {$this->savethumb();$this->displaythumb();} else {$this->displaythumb();}
		}
	...
	function thumbmaker() {
		...
		if ($this->loadimage()) {
			...
			if (count($this->Addtext)>0) {$this->addtext();}
	...
	function addtext() {
		...
		foreach($this->Addtext as $addtext)
		{
			if ($addtext[3]=='') {
				$widthx=imagefontwidth($addtext[4])*strlen($addtext[1]);
				$heighty=imagefontheight($addtext[4]);
				$fontwidth=imagefontwidth($addtext[4]);
			} else {		
				$dimensions=imagettfbbox($addtext[4],0,$addtext[3],$addtext[1]);
				$widthx=$dimensions[2];$heighty=$dimensions[5];
				$dimensions=imagettfbbox($addtext[4],0,$addtext[3],'W');
				$fontwidth=$dimensions[2];
			}
			$cpos=explode(' ',str_replace('%','',$addtext[2]));
			if (count($cpos)>1) {
			...
			if ($addtext[3]=='')
				imagestring($this->im,$addtext[4],$cposx,$cposy,$addtext[1],imagecolorallocate($this->im,hexdec(substr($addtext[5],1,2)),hexdec(substr($addtext[5],3,2)),hexdec(substr($addtext[5],5,2))));
			else
				imagettftext($this->im,$addtext[4],0,$cposx,$cposy,imagecolorallocate($this->im,hexdec(substr($addtext[5],1,2)),hexdec(substr($addtext[5],3,2)),hexdec(substr($addtext[5],5,2))),$addtext[3],$addtext[1]);
		}
	}
